                        ...
                        try {
                            ...
                            try {
                                avatar = document.querySelector(
                                    "#appbar-content .appbar-nav-avatar"
                                ).src;
                            } catch (err) {}
                            var listOfLinks = document.querySelector(
                                "#browse-items-primary script"
                            );
                            if (listOfLinks) {
                                var videoLinks = JSON.parse(
                                    listOfLinks.innerHTML
                                );
                                async.eachSeries(
                                    videoLinks.itemListElement[0].item
                                        .itemListElement,
                                    (item, callback) => {
                                        urls.push(item.url);
                                        callback();
